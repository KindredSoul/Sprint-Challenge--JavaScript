...
function getNames(item, arr, key) {
...
getNames(universities, graduates, "first_name");
...
function getInfo(item, arr, key1, key2) {
  for (let i=0; i<arr.length; i++){
    item.push(arr[i][key1] +" "+ arr[i][key2]);
  }
  return item;
}

getInfo(contactInfo, graduates, "first_name", "email")
...
function findPrefix(item, arr, key) {
  for (let i=0; i<arr.length; i++)
    if(arr[i][key].includes("Uni")){
      item.push(arr[i][key]);
    }
...
findPrefix(uni, graduates, "university")
console.log(uni);
...
const displayNames = (item, arr, key1, key2) => {arr.forEach((arr) => item.push(`Name: ${arr[key1]}, Scientific: ${arr[key2]}`))};

displayNames(animalNames, zooAnimals, "animal_name", "scientific_name");
...
*/
const populationTotal = zooAnimals.reduce((population, currentValue) => population + currentValue.population, 0);
